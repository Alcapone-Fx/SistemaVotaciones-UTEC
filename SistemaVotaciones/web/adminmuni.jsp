...
<body onkeydown="return showKeyCode(event)">
    ...
    <%
        if(request.getParameter("edit")!=null){
            ...
            l1.addAll((Collection)request.getAttribute("mosMun"));
            for(Municipio v:l1){
                if(v.getMunicipio_id()==id){
                    i=v.getMunicipio_id();
                    municipio=v.getMunicipio_municipio();
                }
            }
        }
    %>
	...
	<div class="container-fluid">
 	...
 	<div class="col-md-4 col-lg-9">
            <fieldset>
                <form name="frmDatos" action="ControladorMunicipio" method="post">
                        <h2 style="text-align:center;">Panel de control</h2>
                            <legend>Ingresar Nuevo Municipio</legend>                
                                    <div class="form-group">
                                        <div>
                                            <label>Municipio:</label>
                                            <input type="text" class="form-control input-sm chat-input"  name="municipio" id="municipio" placeholder="Escriba el Municipio" onkeyup="validacion('municipio');" size="50" value="<%if(request.getParameter("edit")!=null){out.print(municipio);}%>" required>
                                            <span class="help-block"></span>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div>
                                            <label>ID de Municipio</label><br>
                                            <input type="text" class="form-control input-sm chat-input"  name="id" id="id" placeholder="Escriba ID del Municipio" onkeyup="validacion('id');" size="50" <%if(request.getParameter("edit")!=null){out.print("value="+i);}%> required>
                                            <span class="help-block"></span> 
                                        </div>
                                    </div>                               				
                ...
                <br>
                <!--<form name="frmMunicipio" action="ControladorMunicipio" method="POST">
            ...
            <fieldset style="margin-top: 30px">
                    ...
                    <table class="table table-striped custab">
                                <thead>
                                    ...
                                    <%
                                        if(request.getAttribute("mosMun")!=null){
                                            ArrayList<Municipio> l1=new ArrayList<Municipio>();
                                            l1.addAll((Collection)request.getAttribute("mosMun"));
                                            for(Municipio v:l1){
                                    %>
                                                <tr><td><%=v.getMunicipio_id()%></td>
                                                    <td><%=v.getMunicipio_municipio()%></td>
                                                    <td>
                                                        <form name="frmEdit" action="ControladorMunicipio" method="post">
                                                        <button class='btn btn-info btn-xs'><span class="glyphicon glyphicon-edit"></span> Edit</button> 
                                                        <input type="hidden" name="edit">
                                                        <input type="hidden" name="ID" value="<%=v.getMunicipio_id()%>" >
                                                        </form>
                                                    </td>
                                                    <td>
                                                        <form name="frmDelete" action="ControladorMunicipio" method="post">
